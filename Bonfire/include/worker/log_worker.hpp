...
public:
	/*
	* Listener logic for the performance logger background thread
	* 
	* @param queueManager& a reference to the QueueManager instance
	* @param handlerManager& a reference to the HandlerManager instance
	* @param delay the delay of the background thread in milliseconds
	*/
	static void PerformanceListener(QueueManager& queueManager, HandlerManager& handlerManager, unsigned int delay);

	/*
	* Listener logic for the custom logger background thread
	*
	* @param queueManager& a reference to the QueueManager instance
	* @param terminalHandler& a reference to the TerminalManager instance
	* @param handlerManager& a reference to the HandlerManager instance
	* @param delay the delay of the background thread in milliseconds
	*/
